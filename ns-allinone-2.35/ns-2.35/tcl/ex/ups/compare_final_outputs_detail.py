...
base_file = open(sys.argv[1]).readlines()                       # orig pcts
lstf_file = open(sys.argv[2]).readlines()                       # lstf pcts
max_flow_id = 0

tmin = dict()                                                   # tmin[(src,dst)] is min time from src to dst in ns.
minTimes = open(sys.argv[3]).readlines()
for line in minTimes:
  words = line.split()
  tmin[(words[1],words[2])] = long(words[0])

out_file_compare = open(sys.argv[4], 'w')                       # compare out
out_file_ratios = open(sys.argv[5], 'w')                        # ratio out

lstf_ts = dict()                                                # lstf_ts[(flow,seqnum)] = pct in ns.
...
count = 0
base_ts = dict()
for line in base_file:                                          # each line in orig pcts.
  words = line.split()
  if(int(words[3]) > max_flow_id):                              # words[3] = flow_id
    max_flow_id = int(words[3])
  key = (int(words[3]), int(words[4]))                          # key = (flow_id, seqnum)
  base = tmin[(words[1], words[2])]                             # min time from src to dst
  pct_base = long(words[0]) - base                              # the base pct
  if(pct_base < 0): #correcting 1ns rounding issues
    pct_base = 0
  try:
    pct_lstf = lstf_ts[key] - base                              # the lstf pct
    if (pct_lstf < 0):
      pct_lstf = 0
