...
for line in lines[:-1]:
    ...
    print >> sys.stderr, sys.argv[1] + " Loop 1 (lines) " + str(i) + '/' + str(len(lines)-1)
    i += 1
...
for key in sorted(delay.keys()):
    ...
    print >> sys.stderr, sys.argv[1] + " Loop 2 (delay) " + str(i) + '/' + str(len(delay))
    i += 1
    ...
    if (key == 1):
        ...
        if (key < max_trans_time):
            ...
            if (key == max_trans_time):
                ...
                buckets[">1"]["Total"] = buckets[">1"]["Total"] + len(
                    delay[key])
                if (key < 2 * max_trans_time):
                    ...
                    if (key == 2 * max_trans_time):
                        ...
                        buckets[">2"]["Total"] = buckets[">2"]["Total"] + len(
                            delay[key])
    ...
    for x in delay[key]:
        ...
        if (x[1] not in buckets[bucketid]["Base Time"].keys()):
            ...
            buckets[bucketid]["Base Time"][
                x[1]] = buckets[bucketid]["Base Time"][x[1]] + 1

        slack_ms = int(float(x[2]) / 1000000)
        if (slack_ms not in buckets[bucketid]["Slack(ms)"].keys()):
            buckets[bucketid]["Slack(ms)"][slack_ms] = 1
        else:
            buckets[bucketid]["Slack(ms)"][
                slack_ms] = buckets[bucketid]["Slack(ms)"][slack_ms] + 1

i = 0

for key in buckets.keys():

    print >> sys.stderr, sys.argv[1] + " Loop 3 (buckets) " + str(i) + '/' + str(len(buckets))
    i += 1

    print "\n" + str(key) + ":"
    for key2 in buckets[key]:
        if (key2 == "Total"):
            print str(key2) + " : " + str(buckets[key][key2]) + " ( " + str(
                float(buckets[key][key2]) * 100 / float(total)) + " ) "
        else:
            if (key2 == "Flows"):
...
print "Flows Delayed " + str(len(flows)) + " = " + str(
    float(len(flows)) * 100 / float(total_flows)) + "%"
print "Total Packets Delayed " + str(total_delayed) + " = " + str(
    float(total_delayed) * 100 / float(total)) + "%"
print "***************************"
