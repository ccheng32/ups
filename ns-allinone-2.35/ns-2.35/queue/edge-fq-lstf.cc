...
edgeFQLstfQueue::edgeFQLstfQueue()
    : srcid_(-1)
    , interSlack_(0)
    , tchan_(0)
{
    ...
    if (curlen_ > 0) {
        ...
        if (!per_flow_map.count(iph->flowid())) {
            ...
            if (((long long int)(Scheduler::instance().clock() * kStoNs) - per_flow_map[iph->flowid()].arrival_time)
                ...
                iph->prio() = per_flow_map[iph->flowid()].orig_slack + (long long int)interSlack_ - ((long long int)(Scheduler::instance().clock() * kStoNs) - per_flow_map[iph->flowid()].arrival_time);
            }
        }
...
void edgeFQLstfQueue::trace(TracedVar* v)
{
    const char* p;
