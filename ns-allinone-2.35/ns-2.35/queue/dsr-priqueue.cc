...
int CMUPriQueue::command(int argc, const char* const* argv)
{
...
void CMUPriQueue::log_stats()
{
	 ...
	 */
    if (s >= qlen_logthresh_ || d >= qlen_logthresh_ || stat_recv_ > fw_logthresh_) {
...
void CMUPriQueue::recv(Packet* p, Handler*)
{
#if PRIQUEUE_DEBUG > 0
    prq_validate();
#endif
...
/* ======================================================================
   ...
   ====================================================================== */
void CMUPriQueue::prq_enqueue(Packet* p)
{
    int q = prq_assign_queue(p);
    struct ifqueue* ifq = &prq_snd_[q];
...
int CMUPriQueue::prq_assign_queue(Packet* p)
{
    struct hdr_cmn* ch = HDR_CMN(p);
